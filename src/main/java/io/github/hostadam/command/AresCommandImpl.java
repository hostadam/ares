...
public class AresCommandImpl extends Command {
    ...
    public AresCommandImpl(CommandRegistry handler, AresCommandData command) {
    ...
    public AresCommandData getSubCommand(String name) {
        AresCommandData data = this.subcommands.get(name.toLowerCase());
        return data != null ? data : this.subcommands.values().stream().filter(aresCommandData -> aresCommandData.getAliases().contains(name)).findAny().orElse(null);
    }
